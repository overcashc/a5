...
class Body(tk.Frame):
    ...
    def set_posts(self, posts:list):
        self._posts = [Post]
        [self.insert_post(i) for i in posts]
...
class MainApp(tk.Frame):
    ...
    def new_profile(self):
        try:
            ...
            print('Did not create a new dsu file')
        self.footer.set_status(self._profile_filename)
        self._current_profile = NaClProfile()
    ...
    def save_profile(self):
        ...
        if self.is_online == 1:
            self.publish(self._current_profile.get_posts()[-1])

        self.body.set_text_entry("")
    '''
    Sends the post to the ds class server
    '''
    def publish(self, post: Post):
        ds_client.send(server='168.235.86.101', port=3021, username='mulan', password='pass123', message=post.get_entry())
        self.footer.set_status('Post Published to Server')
    """
    ...
    def _draw(self):
        ...
        # Added callback for online/offline checkbox
